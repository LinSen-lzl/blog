...
<p>MyISAM是5.5版本之前的默认存储引擎，与InnoDB相比，MyISAM不支持事务、行级锁、MVCC等高级特性，但它具有简单、高效、易于维护等特点，适用于一些对数据完整性要求不高、读多写少的场景</p>
<p>MyISAM的特性：</p>
<ul>
<li>
<p>表级锁：字面意思，表级锁就是对整张表加锁的并发控制机制，与行级锁相比，表级锁也一样<strong>读锁属于共享锁</strong>，<strong>写锁属于排他锁</strong>，在读操作上允许多个线程同时对整张表加读锁并发读取，任何写操作必须等到所有读锁释放才能执行；但在写操作上不同，由于<strong>缺少MVCC支持</strong>，在一个线程持有写锁时，其他线程既不能读也不能写</p>
</li>
<li>
